...
class Config:
    ...
    app_description: str = "Application packaged with easy-installer"
...
def validate_and_normalise(cfg: Config) -> Config:
    ...
    if target_os is None:
        raise ConfigError(
            f"Unsupported OS: {cfg.target_os}. "
            f"Use one of: {', '.join(SUPPORTED_OS)}"
        )
    ...
    if arch is None:
        raise ConfigError(
            f"Unsupported arch: {cfg.arch}. "
            f"Use one of: {', '.join(SUPPORTED_ARCH)}"
        )
    ...
    if target_type is None:
        raise ConfigError(
            f"Unsupported type: {cfg.target_type}. "
            f"Use one of: {', '.join(TYPE_ALIASES)}"
        )
    ...
    if target_type not in allowed:
        raise ConfigError(
            f"Type '{target_type}' is not supported for {target_os}. "
